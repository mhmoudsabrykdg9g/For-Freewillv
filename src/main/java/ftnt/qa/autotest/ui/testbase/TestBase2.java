...
import ftnt.qa.autotest.ui.fortisiem.testcases.LogTest;
...
public abstract  class TestBase2 implements ITestBase {
	...
	public void beforeClass(XmlTest xt, ITestContext tc) throws Exception{
		logger.info("当前测试开始时间是："+tc.getStartDate());
		logger.info("开始运行测试,当前测试类是:" + this.getClass().getName());
	}
	
	@AfterClass()
	public void afterClass(XmlTest xt, ITestContext tc) throws Exception{
		logger.info("测试类：" + this.getClass().getName()+" 执行完毕");
		logger.info("当前测结束时间是："+tc.getEndDate());
	}
	...
	public void cleanEnv(ITestResult rs,XmlTest xt,Method m,ITestContext tc) throws Exception {
		try {
			if (!rs.isSuccess()) {
				...
				logger.info("=====测试用例: " + rs.getMethod().getMethodName() + " 运行失败，原因： "
